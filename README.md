...
```mermaid
graph TD
    A["Anakara Çin Ekosistemi"] --> B["Yönetişim ve Sosyal Kredi"]
    A --> C["Büyük Güvenlik Duvarı (İntranet)"]
    
    B --> B1["996 Çalışma Etiği"]
    B --> B2["Şehir Katman Hiyerarşisi"]
    
    C --> C1["Süper Uygulama Entegrasyonu (WeChat/Douyin)"]
    C --> C2["Yapay Zeka ve Yüz Tanıma"]
    
    B1 --> D["Ekonomik Çıktı"]
    C1 --> D
    
    D --> E["Küresel Etki (Kuşak ve Yol)"]
    
    style A fill:#aa0000,stroke:#333,stroke-width:4px,color:#fff
...
<div align="center">
    <p><i>"Kızıl Ejder'in kodlarını çözüyoruz, her seferinde bir blok."</i></p>
</div>
